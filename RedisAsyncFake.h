#ifndef _REDISASYNCFAKE_H_
#define _REDISASYNCFAKE_H_
...
public:
	...
	//初始化Redis
	bool InitAsyncRedisAndStart(const std::string& host, unsigned short port, const std::string& auth, std::function<void(std::function< void()>)> dispath);

	//投递命令
	void PushCommandWithCallBack(const std::string& command, const CommandCallBack& callback = nullptr);
	void PushCommandsWithCallBack(const std::vector<std::string>& command, const CommandsCallBack& callback = nullptr);
